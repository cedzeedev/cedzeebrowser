...
from PyQt6.QtWidgets import (
    QWidget, QVBoxLayout, QListWidget, QListWidgetItem, QProgressBar,
...
class DownloadItemWidget(QFrame):
    def __init__(self, download_item: QWebEngineDownloadRequest, parent=None):
        ...
        self.filename_label = QLabel(os.path.basename(self.download_item.downloadFileName()))
    ...
    def update_state(self):
        ...
        if state == QWebEngineDownloadRequest.DownloadState.DownloadInProgress:
            ...
            if not self.is_history_item: 
                ...
                while parent_manager is not None and not isinstance(parent_manager, DownloadManager):
    ...
    def _update_progress_from_properties(self):
        ...
        if bytes_total > 0:
            ...
            self.status_label.setText(f"Téléchargé {bytes_received / (1024*1024):.2f} Mo / {bytes_total / (1024*1024):.2f} Mo")
        else:
            self.progress_bar.setRange(0, 0)
    ...
    def open_file(self):
        ...
        if os.path.exists(file_path):
            ...
            self.status_label.setText("Fichier introuvable. Peut-être déplacé ou supprimé.")

    def delete_item(self):
        parent_list_widget = self.parentWidget()
        while parent_list_widget is not None and not isinstance(parent_list_widget, QListWidget):
        ...
        if isinstance(parent_list_widget, QListWidget):
            ...
            if self.is_history_item:
                ...
                while parent_manager is not None and not isinstance(parent_manager, DownloadManager):
                    parent_manager = parent_manager.parentWidget()
                if isinstance(parent_manager, DownloadManager):
                    parent_manager.remove_history_item(self.download_item.downloadFileName())
...
class DownloadManager(QWidget):
    ...
    def add_download(self, download_item: QWebEngineDownloadRequest):
        for i in range(self.list_widget.count()):
            ...
            if isinstance(item_widget, DownloadItemWidget) and item_widget.download_item is download_item:
                return
    ...
    def save_completed_download(self, download_item: QWebEngineDownloadRequest):
        ...
        if os.path.exists(file_path):
            ...
            if not any(item['path'] == download_info['path'] for item in self.completed_downloads_history):
                self.completed_downloads_history.append(download_info)
                self._save_history_to_file()
        else:
            print(f"Avertissement: Le fichier {file_path} n'existe pas après le téléchargement. Il se peut qu'il ait été déplacé ou que le chemin ne soit pas encore à jour.")
    ...
    def load_download_history(self):
        if os.path.exists(self.DOWNLOAD_HISTORY_FILE):
            try:
                ...
                for item_info in self.completed_downloads_history:
                    class DummyDownloadRequest(QObject):
                        ...
                        def __init__(self, filename, path, parent=None):
                            ...
                            self.bytesReceived.emit(self.receivedBytes(), self.totalBytes())
                    ...
                    dummy_download_item = DummyDownloadRequest(item_info["filename"], item_info["path"], parent=self)
                    ...
                    download_widget.status_label.setText(f"Terminé le {item_info['timestamp']}")
                ...
                print(f"Erreur lors du chargement de l'historique des téléchargements: {e}")
                self.completed_downloads_history = []
            except Exception as e:
                print(f"Une erreur inattendue est survenue lors du chargement de l'historique: {e}")
                self.completed_downloads_history = []

    def remove_history_item(self, file_path_to_remove: str):
        self.completed_downloads_history = [
            item for item in self.completed_downloads_history
    ...
    def _save_history_to_file(self):
        try:
            ...
            print(f"Erreur lors de la sauvegarde de l'historique des téléchargements: {e}")